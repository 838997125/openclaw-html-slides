...
5. **页面切换方向** — 页面左右切换，切换按钮放置在下方；页面上下切换，切换按钮放置在右侧（推荐选"左右切换"）
...
预览保存在 `.slide-previews/`（style-a.html / style-b.html / style-c.html），**每个都是独立的完整页面（填满 100vh）**，非嵌入式 iframe。

**生成后必须用 PowerShell `Start-Process` 同时打开全部 3 个文件给用户看（禁止只给路径不开文件）：**
```powershell
powershell -Command "Start-Process 'C:\完整\路径\style-a.html'; Start-Process 'C:\完整\路径\style-b.html'; Start-Process 'C:\完整\路径\style-c.html'"
```
...
- 每个区块添加清晰注释 `/* === 区块名称 === */`
- **JavaScript 必须包含以下核心功能：首次加载时给 slide[0] 加 .visible；IntersectionObserver 监听每个 slide，当 slide 进入视口（threshold:0.5）时给该 slide 加 .visible，触发 CSS .slide.visible .reveal 规则使内容显示；goTo() 翻页用 scrollIntoView() 无 smooth 模式（确保 scroll-snap 正确触发）**
- **CSS 动画核心：`.reveal` 默认 opacity:0；`.slide.visible .reveal` opacity:1 + transform(0)；禁止用 `.reveal.visible` 选择器（必须依赖父级 .slide 的 .visible 类触发）**
...
1. **打开预览** — 使用 PowerShell `Start-Process` 在浏览器打开，禁止只给路径不开文件：
   ```powershell
   powershell -Command "Start-Process 'C:\完整\路径\filename.html'"
   ```
2. **告知用户：**
   - 文件位置、风格名称、页数
